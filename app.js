...
function mealFoodTotals(meal){const items=Array.isArray(meal?.foodItems)?meal.foodItems:[];if(items.length)return items.reduce((a,x)=>{const t=nutritionFromObject(x);return{kcal:a.kcal+t.kcal,protein:a.protein+t.protein,carbs:a.carbs+t.carbs,fat:a.fat+t.fat}},{kcal:0,protein:0,carbs:0,fat:0});return nutritionFromObject(meal)}
function mealHasNutritionLog(meal){const items=Array.isArray(meal?.foodItems)?meal.foodItems:[];if(items.length)return true;const t=nutritionFromObject(meal);return t.kcal>0||t.protein>0||t.carbs>0||t.fat>0}
...
function toggleRecommendedMeal(index){const l=getLog(activeDate),meals=ensureMeals(l,activeDate),m=meals[index];if(!m)return;const items=Array.isArray(m.foodItems)?m.foodItems:[];const planItems=items.filter(x=>x&&x.source==='recommended-plan');const otherItems=items.filter(x=>!x||x.source!=='recommended-plan');if(planItems.length&&otherItems.length===0){m.foodItems=[];}else if(!items.length){const t=calculateNutrition(),count=Math.max(1,meals.length),planText=mealChoices[m.key][m.choice%mealChoices[m.key].length];m.foodItems=[{name:planText,amount:1,unit:'식',kcal:Math.round(t.kcal/count),protein:roundMacro(t.protein/count),carbs:roundMacro(t.carb/count),fat:roundMacro(t.fat/count),source:'recommended-plan',confirmedAt:new Date().toISOString()}];}else{return;}m.done=mealHasNutritionLog(m);syncFoodTotals(l,meals);l.priorities.nutrition=meals.every(mealHasNutritionLog);l.updatedAt=new Date().toISOString();saveState();saveDailyLogNow(activeDate);render()}
...
function renderMeals(log){const t=calculateNutrition(),meals=ensureMeals(log,activeDate),completionChanged=normaliseMealCompletion(meals),eaten=syncFoodTotals(log,meals);if(completionChanged){log.priorities.nutrition=meals.every(mealHasNutritionLog);log.updatedAt=new Date().toISOString();saveState()}nutritionTargetCard.innerHTML=`<div><p class="eyebrow">DAILY NUTRITION</p><h3>${Math.round(eaten.kcal).toLocaleString()} / ${t.kcal.toLocaleString()} kcal</h3><p class="muted">섭취 P ${roundMacro(eaten.protein)}g · C ${roundMacro(eaten.carbs)}g · F ${roundMacro(eaten.fat)}g</p><p class="muted">목표 P ${t.protein}g · C ${t.carb}g · F ${t.fat}g · Water ${t.water.toLocaleString()}ml</p></div><span class="goal-pill">${escapeHtml(state.settings.goalMode)}</span>`;mealPlanList.innerHTML=meals.map((m,i)=>{const mt=mealFoodTotals(m),logged=mealHasNutritionLog(m),planText=mealChoices[m.key][m.choice%mealChoices[m.key].length],items=Array.isArray(m.foodItems)?m.foodItems:[],actualText=items.map(x=>x.name).filter(Boolean).join(' · '),legacyFoodText=items.length&&String(m.customText||'').trim()===actualText,note=String(m.note||(legacyFoodText?'':m.customText)||'').trim(),summary=logged?`${Math.round(mt.kcal)} kcal · P ${roundMacro(mt.protein)}g · C ${roundMacro(mt.carbs)}g · F ${roundMacro(mt.fat)}g`:Math.round(t.kcal/meals.length)+' kcal guideline';return `<article class="meal-plan-card ${logged?'done':''}"><button class="meal-check" data-meal-done="${i}" aria-label="${logged?'Logged meal':'Log meal'}">${logged?'✓':'○'}</button><div class="meal-card-content"><p class="eyebrow">${escapeHtml(localizeMealName(m.key,m.name).toUpperCase())}</p><div class="meal-plan-block"><small class="meal-section-label">${state.settings.language==='ko'?'추천 식단':'PLAN'}</small><h3>${escapeHtml(planText)}</h3><small>${logged?Math.round(t.kcal/meals.length)+' kcal guideline':summary}</small></div>${logged?`<div class="meal-actual-block"><small class="meal-section-label">${state.settings.language==='ko'?'실제 섭취':'ACTUAL'}</small><strong>${escapeHtml(actualText||'기록된 음식')}</strong><small>${summary}</small></div>`:''}${note?`<p class="meal-note">${escapeHtml(note)}</p>`:''}</div><div class="meal-actions"><button class="mini-edit" data-meal-scan="${i}">📷 AI 사진</button><button class="mini-edit" data-meal-food="${i}">음식 검색·합계</button><button class="mini-edit" data-meal-edit="${i}">메모 입력</button><button class="mini-edit" data-meal-change="${i}">추천 변경</button></div></article>`}).join('')}
