...
const shareEls={
  ...
  download:document.getElementById('downloadShareBtn'),nativeShare:document.getElementById('nativeShareBtn'),textSize:document.getElementById('shareTextSize'),routeSize:document.getElementById('shareRouteSize'),logoSize:document.getElementById('shareLogoSize'),photoSize:document.getElementById('sharePhotoSize'),resetLayout:document.getElementById('resetStoryLayoutBtn'),saveLayout:document.getElementById('saveStoryLayoutBtn')
};
...
function openShareCard(id){
  ...
  shareEls.caption.value=state.settings.language==='en'?'Today, I showed up. (ง •̀_•́)ง':'오늘도 해냈다. (ง •̀_•́)ง';sharePhotoImage=null;sharePhotoTransform={x:0,y:0,zoom:1};if(shareEls.photoSize)shareEls.photoSize.value='100';shareEls.photo.value='';renderShareCard();shareEls.dialog.showModal()
}
...
shareEls.canvas.addEventListener('pointerdown',e=>{const p=sharePointerPoint(e),order=['caption','metrics','logo','route','footer'],key=order.find(k=>pointInBounds(p.x,p.y,shareBounds[k]));if(key){shareDrag={type:'element',key,start:p,origin:{...storyLayout()[key]}}}else if(sharePhotoImage){shareDrag={type:'photo',x:e.clientX,y:e.clientY,startX:sharePhotoTransform.x,startY:sharePhotoTransform.y,w:shareEls.canvas.getBoundingClientRect().width,h:shareEls.canvas.getBoundingClientRect().height}}else return;shareEls.canvas.setPointerCapture?.(e.pointerId);shareEls.canvas.classList.add('dragging');e.preventDefault()});
