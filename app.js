...
function dailyFoodTotals(meals){return (Array.isArray(meals)?meals:[]).reduce((a,m)=>{const t=mealFoodTotals(m);return{kcal:a.kcal+t.kcal,protein:a.protein+t.protein,carbs:a.carbs+t.carbs,fat:a.fat+t.fat}},{kcal:0,protein:0,carbs:0,fat:0})}
function roundMacro(n){return Math.round((+n||0)*10)/10}
function hasStructuredNutrition(meals){return (Array.isArray(meals)?meals:[]).some(m=>{if(Array.isArray(m?.foodItems)&&m.foodItems.length)return true;const t=nutritionFromObject(m);return t.kcal>0||t.protein>0||t.carbs>0||t.fat>0})}
function syncFoodTotals(log,meals){if(!hasStructuredNutrition(meals))return{kcal:+log.calories||0,protein:+log.protein||0,carbs:+log.carbs||0,fat:+log.fat||0};const t=dailyFoodTotals(meals);log.calories=Math.round(t.kcal);log.protein=roundMacro(t.protein);log.carbs=roundMacro(t.carbs);log.fat=roundMacro(t.fat);return t}
