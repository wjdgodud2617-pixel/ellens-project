...
const workoutCatalog={
  ...
  cardio:{label:'Cardio',icon:'❤️',plans:{
    ...
    walking:{name:'Walking',meta:'30–60 min · Easy · Recovery cardio',exercises:[ex('wb-ca-walk','Walking',1,45,0,'Cardio · Recovery','Walk at a brisk but conversational pace.','brisk walking workout')]}
  }},
...
function renderMeals(log){const t=calculateNutrition(),meals=ensureMeals(log,activeDate),eaten=syncFoodTotals(log,meals);nutritionTargetCard.innerHTML=`<div><p class="eyebrow">DAILY NUTRITION</p><h3>${Math.round(eaten.kcal).toLocaleString()} / ${t.kcal.toLocaleString()} kcal</h3><p class="muted">섭취 P ${roundMacro(eaten.protein)}g · C ${roundMacro(eaten.carbs)}g · F ${roundMacro(eaten.fat)}g</p><p class="muted">목표 P ${t.protein}g · C ${t.carb}g · F ${t.fat}g · Water ${t.water.toLocaleString()}ml</p></div><span class="goal-pill">${escapeHtml(state.settings.goalMode)}</span>`;mealPlanList.innerHTML=meals.map((m,i)=>{const mt=mealFoodTotals(m),text=m.customText||mealChoices[m.key][m.choice%mealChoices[m.key].length],summary=mt.kcal>0?`${Math.round(mt.kcal)} kcal · P ${roundMacro(mt.protein)}g · C ${roundMacro(mt.carbs)}g · F ${roundMacro(mt.fat)}g`:(m.customText?'직접 입력한 식단':Math.round(t.kcal/meals.length)+' kcal guideline');return `<article class="meal-plan-card ${m.done?'done':''}"><button class="meal-check" data-meal-done="${i}" aria-label="Complete meal">${m.done?'✓':'○'}</button><div><p class="eyebrow">${escapeHtml(localizeMealName(m.key,m.name).toUpperCase())}</p><h3>${escapeHtml(text)}</h3><small>${summary}</small></div><div class="meal-actions"><button class="mini-edit" data-meal-scan="${i}">📷 AI 사진</button><button class="mini-edit" data-meal-food="${i}">음식 검색·합계</button><button class="mini-edit" data-meal-edit="${i}">메모 입력</button><button class="mini-edit" data-meal-change="${i}">추천 변경</button></div></article>`}).join('')}
...
function renderDashboardRunCard(){const el=document.getElementById('dashboardRunCard');if(!el)return;const lang=state.settings.language||'ko',runs=latestRunsForDashboard();if(!runs.length){el.innerHTML=`<div><p class="eyebrow">${lang==='ko'?'오늘의 러닝':'TODAY\'S RUN'}</p><h3>${lang==='ko'?'아직 러닝 기록이 없어요.':'No run yet.'}</h3><p class="muted">${lang==='ko'?'러닝 탭에서 시작해 보세요.':'Open Run to get moving.'}</p></div><button class="text-btn" data-open-run>${lang==='ko'?'러닝 시작':'Start run'}</button>`}else{el.innerHTML=runs.map(r=>{const k=keyFromDate(new Date(r.endedAt)),label=k===todayKey()?(lang==='ko'?'오늘':'Today'):(lang==='ko'?'어제':'Yesterday');return `<button class="dashboard-run-item" data-open-run><span><small>${label}</small><b>${r.activityType==='walk'?(lang==='ko'?'걷기 ':'Walk '):(lang==='ko'?'러닝 ':'Run ')}${r.distanceKm.toFixed(2)} km</b></span><span>${formatClock(r.durationMs)} · ${paceText(r.avgPaceSecKm)}/km</span></button>`}).join('')}el.querySelectorAll('[data-open-run]').forEach((b,i)=>b.onclick=()=>{const run=runs[i];if(run)openShareCard(run.id)});const allBtn=document.getElementById('dashboardViewRunsBtn');if(allBtn)allBtn.onclick=()=>{switchView('progress');document.getElementById('runHistoryArchive')?.scrollIntoView({behavior:'smooth',block:'start'})}}
...
function renderToday(){const log=getLog(activeDate);renderDashboardRunCard();renderDailyReview(log);autoPriorities(log);const score=scoreFor(log),m=mood(score),d=dateFromKey(activeDate),isToday=activeDate===todayKey();scoreValue.textContent=`${score}%`;scoreEmoji.textContent=m.emoji;coachLine.textContent=m.line;scoreRing.style.setProperty('--score',score);scoreRing.querySelector('span').textContent=score;const lang=state.settings.language||'ko';workoutDayLabel.textContent=isToday?(lang==='ko'?'오늘':'TODAY'):new Intl.DateTimeFormat(lang==='ko'?'ko-KR':'en',{weekday:'long'}).format(d).toUpperCase();workoutDateLabel.textContent=new Intl.DateTimeFormat(lang==='ko'?'ko-KR':'en',{month:'short',day:'numeric',year:'numeric'}).format(d);routineName.textContent=localizeWorkoutName(log.planName||weeklyPlan[d.getDay()].name);workoutHeading.textContent=isToday?(lang==='ko'?'오늘의 운동':"Today's workout"):(lang==='ko'?'선택한 날짜의 운동':'Day workout');todayWorkoutBtn.hidden=isToday;renderMeals(log);renderLatestRunAnalysis();
 const defs=lang==='ko'?[['workout','운동','모든 운동 완료','🏋️'],['nutrition','영양','칼로리와 단백질 기록','🥗'],['water','수분',`${log.water}/${state.settings.waterGoal} ml`,'💧'],['sleep','수면',`${log.sleep}/${state.settings.sleepGoal}시간`,'🌙']]:[['workout','Workout','Complete every exercise','🏋️'],['nutrition','Nutrition','Log calories and protein','🥗'],['water','Water',`${log.water}/${state.settings.waterGoal} ml`,'💧'],['sleep','Sleep',`${log.sleep}/${state.settings.sleepGoal} h`,'🌙']];priorityGrid.innerHTML=defs.map(([k,t,s,e])=>`<button class="priority-card ${log.priorities[k]?'done':''}" data-priority="${k}"><span>${e}</span><b>${t}</b><small>${s}</small></button>`).join('');
 workoutList.innerHTML=log.exercises.length?log.exercises.map((x,i)=>`<article class="exercise-card ${x.done?'done':''}"><input class="check" type="checkbox" data-ex-check="${i}" ${x.done?'checked':''} aria-label="Complete ${escapeHtml(x.name)}"><button class="text-btn exercise-open" data-ex-open="${i}" style="text-align:left"><h3>${escapeHtml(localizeWorkoutName(x.name))}</h3><p>${x.sets}${lang==='ko'?'세트':' sets'} × ${x.reps}${lang==='ko'?'회':' reps'}${x.weight?` · ${x.weight} kg`:''}</p></button><button class="mini-edit" data-ex-edit="${i}" aria-label="${lang==='ko'?'수정':'Edit'} ${escapeHtml(localizeWorkoutName(x.name))}">${lang==='ko'?'수정':'Edit'}</button></article>`).join(''):`<div class="empty-state"><span>🧘</span><p>${lang==='ko'?'예정된 운동이 없습니다.':'No exercises scheduled.'}</p><button class="text-btn" data-empty-add>${lang==='ko'?'+ 운동 추가':'+ Add an exercise'}</button></div>`;
...
function renderDaySummary(){const log=state.logs[selectedDate],s=log?scoreFor(log):0,m=mood(s),routine=weeklyPlan[dateFromKey(selectedDate).getDay()];daySummary.innerHTML=`<p class="eyebrow">${selectedDate}</p><h2>${escapeHtml(localizeWorkoutName(log?.planName||routine.name))}</h2><p class="muted">${m.emoji} ${s}% ${state.settings.language==='ko'?'완료':'complete'} · ${log?.exercises?.length??routine.exercises.length}${state.settings.language==='ko'?'개 운동':' exercises'}</p>${log?`<p>${state.settings.language==='ko'?'수분':'Water'} ${log.water||0} ml · ${state.settings.language==='ko'?'수면':'Sleep'} ${log.sleep||0} h · ${state.settings.language==='ko'?'단백질':'Protein'} ${log.protein||0} g</p>`:`<p>${state.settings.language==='ko'?'이 날짜를 열면 주간 운동 계획이 생성됩니다.':'The weekly plan will be created when you open this day.'}</p>`}<button class="primary-btn" id="openSelectedDay">${state.settings.language==='ko'?'이 운동 열기':'Open this workout'}</button>`;document.getElementById('openSelectedDay').onclick=()=>{activeDate=selectedDate;switchView('today');renderToday()}}
...
const KO_WORKOUT_NAMES={
 ...
 'Run + Mobility':'러닝·모빌리티','Easy Run':'가벼운 러닝','Running Strides':'러닝 스트라이드','Hip & Ankle Mobility':'고관절·발목 가동성','Recovery Run':'회복 러닝','Tempo Run':'템포 러닝','Interval Run':'인터벌 러닝','Long Run':'롱 러닝','Hill Run':'언덕 러닝',
 ...
 'CrossFit Metcon':'크로스핏 메트콘','CrossFit Kettlebell':'크로스핏 케틀벨','CrossFit Bodyweight WOD':'크로스핏 맨몸 WOD','CrossFit Row & SkiErg':'크로스핏 로잉·스키에르그',
...
shareEls.canvas.addEventListener('pointerdown',e=>{const p=sharePointerPoint(e),target=shareEls.editTarget?.value||'auto',order=['caption','metrics','logo','route'];let key=null;if(target==='auto')key=order.find(k=>pointInBounds(p.x,p.y,shareBounds[k]));else if(target!=='photo')key=target;if(key&&shareBounds[key]){shareDrag={type:'element',key,start:p,origin:{...storyLayout()[key]}}}else if(sharePhotoImage&&(target==='photo'||target==='auto')){shareDrag={type:'photo',x:e.clientX,y:e.clientY,startX:sharePhotoTransform.x,startY:sharePhotoTransform.y,w:shareEls.canvas.getBoundingClientRect().width,h:shareEls.canvas.getBoundingClientRect().height}}else return;shareEls.canvas.setPointerCapture?.(e.pointerId);shareEls.canvas.classList.add('dragging');e.preventDefault()});
...
function openWorkoutStory(){syncWorkoutStoryDetails(true);workoutStoryPhotoTransform={x:0,y:0,zoom:(+workoutStory.photoSize?.value||100)/100};updateWorkoutStoryLabels();drawWorkoutStory();workoutStory.dialog.showModal()}
