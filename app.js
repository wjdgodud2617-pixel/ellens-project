...
function exerciseMeasureType(x){
  const n=String(x?.name||'').toLowerCase();
  if(x?.gpsActivity)return 'gps';
  if(/farmer.?s? carry|farmer carry|sled push|sled pull|ski ?erg|row(?:ing)?(?:\s|$)|run(?:\s|$)|walking lunges?/.test(n))return 'distance';
  if(/plank|hold|dead hang|wall sit/.test(n))return 'time';
  if(/squat|push[ -]?up|burpee|mountain climber|sit[ -]?up|crunch|lunge|step[ -]?up|pull[ -]?up|chin[ -]?up/.test(n)&&!/(dumbbell|barbell|goblet|kettlebell|weighted|sandbag)/.test(n))return 'bodyweight';
  return 'reps';
}
function formatExercisePrescription(x){
  const ko=(state.settings.language||'ko')==='ko',sets=Math.max(1,+x?.sets||1),reps=Math.max(0,+x?.reps||0),weight=Math.max(0,+x?.weight||0),type=exerciseMeasureType(x);
  if(type==='gps'&&x?.instructions)return escapeHtml(x.instructions);
  const setText=ko?`${sets}세트`:`${sets} set${sets===1?'':'s'}`;
  if(type==='distance'){
    const distanceText=reps>=1000&&reps%1000===0?`${reps/1000} km`:`${reps} m`;
    return `${setText} × ${distanceText}${weight?` · ${weight} kg`:''}`;
  }
  if(type==='time')return `${setText} × ${reps}${ko?'초':' sec'}`;
  if(type==='bodyweight')return `${setText} × ${reps}${ko?'회':' reps'}`;
  return `${setText} × ${reps}${ko?'회':' reps'}${weight?` · ${weight} kg`:''}`;
}
function exerciseDetailUnits(x){
  const ko=(state.settings.language||'ko')==='ko',type=exerciseMeasureType(x);
  if(type==='distance')return {value:Math.max(0,+x.reps||0),label:(+x.reps||0)>=1000&&(+x.reps||0)%1000===0?'km':'m',weight:true};
  if(type==='time')return {value:Math.max(0,+x.reps||0),label:ko?'초':'sec',weight:false};
  return {value:Math.max(0,+x.reps||0),label:ko?'회':'Reps',weight:type!=='bodyweight'};
}
...
function renderRunMode(){if(!runModeEls.overlay)return;const r=runSession,d=r?r.distanceM/1000:0,ms=r?elapsedMs():0,avgMs=ms,avgSpeed=d>0&&avgMs>0?d/(avgMs/3600000):0;const roundedMetres=Math.round(d*1000),showKm=roundedMetres>=1000;runModeEls.distance.textContent=showKm?d.toFixed(2):roundedMetres;const modeUnit=document.getElementById('runModeDistanceUnit');if(modeUnit)modeUnit.textContent=showKm?'KM':'M';runModeEls.time.textContent=formatClock(ms);runModeEls.pace.textContent=avgSpeed.toFixed(1);runModeEls.gps.textContent=!r?.gpsEnabled?(state.settings.language==='ko'?'GPS 꺼짐':'GPS OFF'):r?.hasFix?(state.settings.language==='ko'?'GPS 연결':'GPS LIVE'):(state.settings.language==='ko'?'GPS 대기':'GPS WAITING');runModeEls.gps.className='run-mode-gps '+(r?.hasFix?'live':'');runModeEls.accuracy.textContent=`정확도 ${r?.accuracy?Math.round(r.accuracy):'--'}m`;runModeEls.status.textContent=r?.autoPaused?'자동 일시정지 중':r?.status==='paused'?'러닝 일시정지':r?.hasFix?'GPS 경로 기록 중':'GPS 신호를 찾는 중';runModeEls.pause.textContent=r?.status==='paused'?'▶ 다시 시작':'Ⅱ 일시정지'}
const shareEls={
  ...
  download:document.getElementById('downloadShareBtn'),nativeShare:document.getElementById('nativeShareBtn'),routeSize:document.getElementById('shareRouteSize'),logoSize:document.getElementById('shareLogoSize'),photoSize:document.getElementById('sharePhotoSize'),editTarget:document.getElementById('shareEditTarget'),routeSizeValue:document.getElementById('shareRouteSizeValue'),logoSizeValue:document.getElementById('shareLogoSizeValue'),photoSizeValue:document.getElementById('sharePhotoSizeValue'),positionX:document.getElementById('sharePositionX'),positionY:document.getElementById('sharePositionY'),positionXValue:document.getElementById('sharePositionXValue'),positionYValue:document.getElementById('sharePositionYValue'),resetLayout:document.getElementById('resetStoryLayoutBtn'),saveLayout:document.getElementById('saveStoryLayoutBtn'),showSplits:document.getElementById('shareSplitsToggle'),splitsStatus:document.getElementById('shareSplitsStatus'),splitsSize:document.getElementById('shareSplitsSize'),splitsSizeValue:document.getElementById('shareSplitsSizeValue'),
  metricSizes:{
...
function formatDistance(distanceKm,{compact=false}={}){const km=Math.max(0,Number(distanceKm)||0),metres=Math.round(km*1000);if(metres<1000)return `${metres} m`;return `${km.toFixed(2)} km`}
...
function renderRun(){
  ...
  if(runEls.history) runEls.history.innerHTML=history.length?history.map(x=>`<div class="run-history-card"><button class="run-history-main" data-open-run-detail="${x.id}"><div><h3>${x.activityType==='walk'?(state.settings.language==='ko'?'걷기':'Walk'):(state.settings.language==='ko'?'러닝':'Run')} · ${new Date(x.endedAt).toLocaleDateString(undefined,{month:'short',day:'numeric',year:'numeric'})}</h3><p>${formatClock(x.durationMs)} · ${paceText(runAveragePace(x))}/km</p></div><strong class="history-distance">${formatDistance(x.distanceKm)}</strong></button><button class="mini-edit share-run-btn" data-run-id="${x.id}">${state.settings.language==='ko'?'인증샷 만들기':'Create story'}</button></div>`).join(''):'<div class="empty-state"><span>👟</span><p>No completed runs yet.</p></div>';
...
function runNoticeBody(){
  ...
  const km=((runSession.distanceM||0)/1000).toFixed(2),time=formatClock(elapsedMs()),avgMs=runSession.autoPauseEnabled?movingMs():elapsedMs(),pace=paceText(km>0?(avgMs/1000)/(+km):Infinity);
...
async function drawRouteMap(ctx,route,x,y,w,h,radius=28,splits=[],showSplits=false){
  ...
  marker(points[0],'S','#b9ff3f');
  if(showSplits&&Array.isArray(splits)){
    splits.filter(sp=>Number.isFinite(+sp.lat)&&Number.isFinite(+sp.lon)&&Number.isFinite(+sp.km)).forEach(sp=>{
      let best=0,bestD=Infinity;view.clean.forEach((rp,i)=>{const d=(+rp.lat-+sp.lat)**2+(+rp.lon-+sp.lon)**2;if(d<bestD){bestD=d;best=i}});
      if(points[best])marker(points[best],`${Math.round(+sp.km)}K`,'#b9ff3f');
    });
  }
  marker(points[points.length-1],'F','#ffffff');ctx.textAlign='left';ctx.textBaseline='alphabetic';ctx.restore();return true
}
...
function drawRouteOverlay(ctx,route,x,y,w,h,transparent=false,splits=[],showSplits=false){
  ...
  const marker=(p,label,finish=false)=>{const radius=Math.max(12,w*.037);ctx.beginPath();ctx.arc(p.x,p.y,radius,0,Math.PI*2);ctx.fillStyle=finish?'rgba(255,255,255,.96)':'#b9ff3f';ctx.fill();ctx.lineWidth=transparent?2:4;ctx.strokeStyle=transparent?'rgba(255,255,255,.92)':'rgba(0,0,0,.75)';ctx.stroke();ctx.fillStyle='#071007';ctx.font=`900 ${Math.max(12,Math.round(w*.038))}px system-ui`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(label,p.x,p.y+1)};marker(pts[0],'S');
  if(showSplits&&Array.isArray(splits)){
    const clean=(route||[]).filter(p=>Number.isFinite(+p.lat)&&Number.isFinite(+p.lon));
    splits.filter(sp=>Number.isFinite(+sp.lat)&&Number.isFinite(+sp.lon)&&Number.isFinite(+sp.km)).forEach(sp=>{let best=0,bestD=Infinity;clean.forEach((rp,i)=>{const d=(+rp.lat-+sp.lat)**2+(+rp.lon-+sp.lon)**2;if(d<bestD){bestD=d;best=i}});if(pts[best])marker(pts[best],`${Math.round(+sp.km)}K`)});
  }
  marker(pts[pts.length-1],'F',true);ctx.restore();return true;
}
...
async function renderShareCard(){
  ...
  if(showSplits){
    ...
    if(splits.length){
      ...
      ctx.save();ctx.shadowBlur=0;ctx.fillStyle='rgba(5,8,6,.58)';ctx.beginPath();ctx.roundRect(x-w*.02,y-h*.035,panelW,panelH,18);ctx.fill();ctx.fillStyle='rgba(255,255,255,.74)';ctx.font=`800 ${Math.round(w*.017*scale)}px system-ui`;ctx.textAlign='left';ctx.fillText(state.settings.language==='ko'?'1KM 구간':'1KM SPLITS',x,y);
...
function openShareCard(id){
  ...
  const hasSplits=Array.isArray(shareRunRecord.splits)&&shareRunRecord.splits.some(sp=>Number.isFinite(+sp.km)&&Number.isFinite(+sp.seconds));
  if(shareEls.showSplits){shareEls.showSplits.disabled=!hasSplits;shareEls.showSplits.checked=hasSplits&&(state.settings.shareShowSplits!==false)}
  if(shareEls.splitsStatus)shareEls.splitsStatus.textContent=hasSplits?((state.settings.language==='ko'?`${shareRunRecord.splits.length}개 구간 기록을 불러왔어요.`:`${shareRunRecord.splits.length} split records loaded.`)):(state.settings.language==='ko'?'이 기록에는 저장된 1KM 구간이 없어요.':'No saved 1KM splits for this activity.');
  if(shareEls.splitsSize)shareEls.splitsSize.value=Math.round((storyLayout().splits?.scale||.78)*100);
  if(shareEls.splitsSizeValue&&shareEls.splitsSize)shareEls.splitsSizeValue.textContent=`${shareEls.splitsSize.value}%`;
...
shareEls.showSplits?.addEventListener('change',()=>{state.settings.shareShowSplits=!!shareEls.showSplits.checked;saveState();renderShareCard()});
shareEls.splitsSize?.addEventListener('input',()=>{state.settings.storyLayout=state.settings.storyLayout||{};const current=storyLayout().splits||defaultStoryLayout.splits;state.settings.storyLayout.splits={...current,scale:Math.max(.5,Math.min(1.5,(+shareEls.splitsSize.value||78)/100))};if(shareEls.splitsSizeValue)shareEls.splitsSizeValue.textContent=`${shareEls.splitsSize.value}%`;setStoryMoveTarget('splits');queueStoryRender()});
shareEls.splitsSize?.addEventListener('change',()=>saveState());
...
const storyMovableKeys=['distance','pace','time','caption','route','logo','splits'];
...
function workoutStoryDetailText(){
  ...
  const lines=list.map(x=>`${localizeWorkoutName(x.name)} ${formatExercisePrescription(x)}`);
