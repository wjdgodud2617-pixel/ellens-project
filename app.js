...
function persistStateLocal(){const serialised=JSON.stringify(state);localStorage.setItem(STORAGE_KEY,serialised);localStorage.setItem(STORAGE_BACKUP_KEY,serialised)}
...
function validMealFoodItems(meal){const key=String(meal?.key||'');return (Array.isArray(meal?.foodItems)?meal.foodItems:[]).filter(x=>{if(!x)return false;if(x.source==='recommended-plan')return !!key&&x.mealKey===key;return true})}
function sanitiseMealFoodItems(meals){let changed=false;for(const m of (Array.isArray(meals)?meals:[])){if(!Array.isArray(m?.foodItems))continue;const valid=validMealFoodItems(m);if(valid.length!==m.foodItems.length){m.foodItems=valid;changed=true}}return changed}
function mealFoodTotals(meal){const items=validMealFoodItems(meal);if(items.length)return items.reduce((a,x)=>{const t=nutritionFromObject(x);return{kcal:a.kcal+t.kcal,protein:a.protein+t.protein,carbs:a.carbs+t.carbs,fat:a.fat+t.fat}},{kcal:0,protein:0,carbs:0,fat:0});return{kcal:0,protein:0,carbs:0,fat:0}}
function mealHasNutritionLog(meal){return validMealFoodItems(meal).length>0}
function normaliseMealCompletion(meals){let changed=sanitiseMealFoodItems(meals);for(const m of (Array.isArray(meals)?meals:[])){const shouldBeDone=mealHasNutritionLog(m);if(!!m.done!==shouldBeDone){m.done=shouldBeDone;changed=true}}return changed}
...
function saveMealFoods(){const l=getLog(activeDate),meals=ensureMeals(l,activeDate),m=meals[editingMealIndex];m.done=mealHasNutritionLog(m);syncFoodTotals(l,meals);l.priorities.nutrition=meals.every(mealHasNutritionLog);l.updatedAt=new Date().toISOString();persistStateLocal();saveDailyLogNow(activeDate);foodDialog.close();render()}
function toggleRecommendedMeal(index){const l=getLog(activeDate),meals=ensureMeals(l,activeDate),m=meals[index];if(!m)return;sanitiseMealFoodItems(meals);const items=validMealFoodItems(m),planItems=items.filter(x=>x.source==='recommended-plan'&&x.mealKey===m.key),otherItems=items.filter(x=>x.source!=='recommended-plan');if(planItems.length&&otherItems.length===0){m.foodItems=[];}else if(!items.length){const t=calculateNutrition(),count=Math.max(1,meals.length),planText=mealChoices[m.key][m.choice%mealChoices[m.key].length];m.foodItems=[{name:planText,amount:1,unit:'식',kcal:Math.round(t.kcal/count),protein:roundMacro(t.protein/count),carbs:roundMacro(t.carb/count),fat:roundMacro(t.fat/count),source:'recommended-plan',mealKey:m.key,entryId:crypto.randomUUID(),confirmedAt:new Date().toISOString()}];}else{return;}m.done=mealHasNutritionLog(m);syncFoodTotals(l,meals);l.priorities.nutrition=meals.every(mealHasNutritionLog);l.updatedAt=new Date().toISOString();persistStateLocal();saveDailyLogNow(activeDate);render()}
...
function renderMeals(log){const t=calculateNutrition(),meals=ensureMeals(log,activeDate),completionChanged=normaliseMealCompletion(meals),eaten=syncFoodTotals(log,meals);if(completionChanged){log.priorities.nutrition=meals.every(mealHasNutritionLog);log.updatedAt=new Date().toISOString();saveState()}nutritionTargetCard.innerHTML=`<div><p class="eyebrow">DAILY NUTRITION</p><h3>${Math.round(eaten.kcal).toLocaleString()} / ${t.kcal.toLocaleString()} kcal</h3><p class="muted">섭취 P ${roundMacro(eaten.protein)}g · C ${roundMacro(eaten.carbs)}g · F ${roundMacro(eaten.fat)}g</p><p class="muted">목표 P ${t.protein}g · C ${t.carb}g · F ${t.fat}g · Water ${t.water.toLocaleString()}ml</p></div><span class="goal-pill">${escapeHtml(state.settings.goalMode)}</span>`;mealPlanList.innerHTML=meals.map((m,i)=>{const mt=mealFoodTotals(m),logged=mealHasNutritionLog(m),planText=mealChoices[m.key][m.choice%mealChoices[m.key].length],items=validMealFoodItems(m),actualText=items.map(x=>x.name).filter(Boolean).join(' · '),legacyFoodText=items.length&&String(m.customText||'').trim()===actualText,note=String(m.note||(legacyFoodText?'':m.customText)||'').trim(),summary=logged?`${Math.round(mt.kcal)} kcal · P ${roundMacro(mt.protein)}g · C ${roundMacro(mt.carbs)}g · F ${roundMacro(mt.fat)}g`:Math.round(t.kcal/meals.length)+' kcal guideline';return `<article class="meal-plan-card ${logged?'done':''}"><button class="meal-check" data-meal-done="${i}" aria-label="${logged?'Logged meal':'Log meal'}">${logged?'✓':'○'}</button><div class="meal-card-content"><p class="eyebrow">${escapeHtml(localizeMealName(m.key,m.name).toUpperCase())}</p><div class="meal-plan-block"><small class="meal-section-label">${state.settings.language==='ko'?'추천 식단':'PLAN'}</small><h3>${escapeHtml(planText)}</h3><small>${logged?Math.round(t.kcal/meals.length)+' kcal guideline':summary}</small></div>${logged?`<div class="meal-actual-block"><small class="meal-section-label">${state.settings.language==='ko'?'실제 섭취':'ACTUAL'}</small><strong>${escapeHtml(actualText||'기록된 음식')}</strong><small>${summary}</small></div>`:''}${note?`<p class="meal-note">${escapeHtml(note)}</p>`:''}</div><div class="meal-actions"><button class="mini-edit" data-meal-scan="${i}">📷 AI 사진</button><button class="mini-edit" data-meal-food="${i}">음식 검색·합계</button><button class="mini-edit" data-meal-edit="${i}">메모 입력</button><button class="mini-edit" data-meal-change="${i}">추천 변경</button></div></article>`}).join('')}
...
nutritionLabelEls.save.onclick=()=>{
  if(!nutritionLabelData)return;const idx=+nutritionLabelEls.meal.value,log=getLog(activeDate),meals=ensureMeals(log,activeDate),meal=meals[idx];if(!meal)return alert('저장할 식사를 선택해 주세요.');const y=nutritionLabelScaled();meal.foodItems=meal.foodItems||[];meal.foodItems.push({name:nutritionLabelData.productName||'영양정보표 제품',amount:y.amount,unit:y.unit,kcal:y.kcal,protein:y.protein,carbs:y.carbs,fat:y.fat,sugars:y.sugars,sodium:y.sodium,source:'nutrition-label',mealKey:meal.key,confidence:nutritionLabelData.confidence||0,scannedAt:new Date().toISOString()});meal.done=true;syncFoodTotals(log,meals);log.priorities.nutrition=meals.every(mealHasNutritionLog);log.updatedAt=new Date().toISOString();persistStateLocal();saveDailyLogNow(activeDate);nutritionLabelEls.dialog.close();render();alert(`${meal.name} 식단에 영양정보를 저장했어요.`)
};
