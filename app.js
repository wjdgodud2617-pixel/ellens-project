...
shareEls.canvas?.addEventListener('pointerdown',e=>{const p=sharePointerPoint(e),target=shareEls.editTarget?.value||'auto',order=['caption','time','pace','distance','logo','route'];let key=null;if(target==='auto')key=order.find(k=>pointInBounds(p.x,p.y,shareBounds[k]));else if(target!=='photo')key=target;if(key&&shareBounds[key]){shareDrag={type:'element',key,start:p,origin:{...storyLayout()[key]}}}else if(sharePhotoImage&&(target==='photo'||target==='auto')){shareDrag={type:'photo',x:e.clientX,y:e.clientY,startX:sharePhotoTransform.x,startY:sharePhotoTransform.y,w:shareEls.canvas.getBoundingClientRect().width,h:shareEls.canvas.getBoundingClientRect().height}}else return;shareEls.canvas.setPointerCapture?.(e.pointerId);shareEls.canvas.classList.add('dragging');e.preventDefault()});
