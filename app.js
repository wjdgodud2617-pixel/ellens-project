...
mealPlanList.onclick=e=>{const c=e.target.closest('[data-meal-change]'),d=e.target.closest('[data-meal-done]'),edit=e.target.closest('[data-meal-edit]'),food=e.target.closest('[data-meal-food]'),scan=e.target.closest('[data-meal-scan]'),l=getLog(activeDate),meals=ensureMeals(l,activeDate);if(scan){openFoodScan(+scan.dataset.mealScan);return}else if(food){openFoodEditor(+food.dataset.mealFood);return}else if(edit){const m=meals[+edit.dataset.mealEdit],value=prompt(`${m.name} 식단 메모를 입력하세요.`,m.note||'');if(value===null)return;m.note=value.trim();if(!m.note)delete m.note}else if(c){const m=meals[+c.dataset.mealChange];m.choice=(m.choice+1)%mealChoices[m.key].length}else if(d){const idx=+d.dataset.mealDone,m=meals[idx];if(!mealHasNutritionLog(m)){alert(state.settings.language==='ko'?'음식을 기록하면 자동으로 완료 처리돼요.':'Log food first. The meal will be completed automatically.')}openFoodEditor(idx);return}else return;syncFoodTotals(l,meals);l.priorities.nutrition=meals.every(mealHasNutritionLog);l.updatedAt=new Date().toISOString();saveState();saveDailyLogNow(activeDate);render()}
