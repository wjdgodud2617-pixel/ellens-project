...
function getLog(date=activeDate){
  if(!state.logs[date]){
    state.logs[date]={priorities:{workout:false,nutrition:false,water:false,sleep:false},water:0,sleep:0,calories:0,protein:0,memo:'',exercises:plannedExercises(date),planInitialized:true,planSource:'weekly',updatedAt:new Date().toISOString()};
  }else{
    ...
    if((!Array.isArray(log.exercises)||log.exercises.length===0||isLegacyDefaults(log.exercises))&&log.planSource!=='custom-empty'){
      log.exercises=plannedExercises(date);log.planInitialized=true;log.planSource='weekly';log.updatedAt=new Date().toISOString();localStorage.setItem(STORAGE_KEY,JSON.stringify(state));
    }
  }
...
function renderToday(){const log=getLog(activeDate);autoPriorities(log);const score=scoreFor(log),m=mood(score),d=dateFromKey(activeDate),isToday=activeDate===todayKey();scoreValue.textContent=`${score}%`;scoreEmoji.textContent=m.emoji;coachLine.textContent=m.line;scoreRing.style.setProperty('--score',score);scoreRing.querySelector('span').textContent=score;workoutDayLabel.textContent=isToday?'TODAY':new Intl.DateTimeFormat('en',{weekday:'long'}).format(d).toUpperCase();workoutDateLabel.textContent=new Intl.DateTimeFormat('en',{month:'short',day:'numeric',year:'numeric'}).format(d);routineName.textContent=weeklyPlan[d.getDay()].name;workoutHeading.textContent=isToday?"Today's workout":"Day workout";todayWorkoutBtn.hidden=isToday;renderMeals(log);
