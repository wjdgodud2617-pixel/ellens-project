...
function progressRuns(){return (state.runs||[]).filter(r=>(r.activityType||'run')==='run').slice().sort((a,b)=>new Date(b.endedAt)-new Date(a.endedAt))}
function runHistoryRow(r){return `<div class="progress-run-row"><span><b>${new Date(r.endedAt).toLocaleDateString()}</b><small>${formatClock(r.durationMs)} · ${paceText(r.avgPaceSecKm)}/km</small></span><strong>${formatDistance(r.distanceKm)}</strong></div>`}
function ensureRunHistoryDialog(){if(document.getElementById('runHistoryDialog'))return;document.body.insertAdjacentHTML('beforeend',`<dialog id="runHistoryDialog" class="modal run-history-modal"><div class="section-head"><div><p class="eyebrow">RUNNING HISTORY</p><h2 id="runHistoryDialogTitle">전체 러닝 기록</h2></div><button id="runHistoryDialogClose" class="secondary-btn inline-btn" type="button">닫기</button></div><div id="runHistoryDialogList" class="progress-run-history"></div></dialog>`);document.getElementById('runHistoryDialogClose').onclick=()=>document.getElementById('runHistoryDialog').close()}
