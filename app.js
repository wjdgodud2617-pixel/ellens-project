...
const KO_CATEGORY_NAMES={Strength:'근력',Running:'러닝',Cardio:'유산소',HYROX:'하이록스',Conditioning:'컨디셔닝',Recovery:'회복'};
...
const defaultWorkoutStoryLayout={title:{x:.065,y:.64},summary:{x:.065,y:.72},details:{x:.065,y:.79},caption:{x:.065,y:.94},watermark:{x:.93,y:.965}};
...
function drawWorkoutStory(){
  ...
  const wx=L.watermark.x*w,wy=L.watermark.y*h;ctx.textAlign='right';ctx.fillStyle='#39ff14';ctx.font=`900 ${Math.round(28*scale)}px system-ui`;ctx.fillText('ELDYN',wx,wy);workoutStoryBounds.watermark={x:wx-150*scale,y:wy-40*scale,w:160*scale,h:55*scale};ctx.textAlign='left';ctx.shadowBlur=0
}
...
document.getElementById('renderWorkoutStoryBtn')?.addEventListener('click',drawWorkoutStory);document.getElementById('downloadWorkoutStoryBtn')?.addEventListener('click',()=>{drawWorkoutStory();const a=document.createElement('a');a.href=workoutStory.canvas.toDataURL('image/png');a.download='ELDYN-WORKOUT-STORY.png';a.click()});
